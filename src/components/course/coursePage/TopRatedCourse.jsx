...
import { topRatedCourseList } from "../../../redux/actions/course/CourseShop";
function TopRatedCourse() {
  const { topRatedCourseData } = useSelector((state) => state.courseValue);
  ...
  useMemo(() => {
    dispatch(topRatedCourseList());
  }, []);
  let courses = "";

  if (topRatedCourseData?.data?.data?.length > 0) {
    courses = topRatedCourseData?.data?.data.map((course, index) => (
