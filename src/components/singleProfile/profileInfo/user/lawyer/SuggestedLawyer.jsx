...
export const SuggestedLawyerList = function () {
  let dispatch = useDispatch();
  const [agents, setagents] = useState();
  let getagentListData = async () => {
    let res = await LawyerService.LawyerSuggested();
    // console.log("res :>> ", res);
    if (res.status === 200) {
      setagents(res.data);
    }
...
export const MemoSuggestedLawyerList = React.memo(SuggestedLawyerList);
